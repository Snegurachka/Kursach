...
//    }
    ...
    public static <T> void printList(String filename, List<T> list) {
        try {
            PrintWriter out = new PrintWriter(new File("gnuplot", filename));
            for(int i = 0; i < list.size(); ++i) {
                out.println(i + " " + list.get(i));
            }
            out.close();
        } catch (IOException e) {
            System.err.println("Error in gnuplot printList");
            System.err.println(e);
        }
    }

    public static void printListComplex(String filename, List<Complex> list) {
        try {
            PrintWriter out = new PrintWriter(new File("gnuplot", filename));
            for(int i = 0; i < list.size(); ++i) {
                // TODO вот тут выбираешь чего ты хочешь делать с комплексом
                out.println(i + " " + list.get(i).abs());
            }
            out.close();
        } catch (IOException e) {
            System.err.println("Error in gnuplot printList");
